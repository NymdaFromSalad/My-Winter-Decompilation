...
public class MasterAudio : MonoBehaviour
{
	...
	public const string GroupTemplateFolder = "Assets/DarkTonic/MasterAudio/Sources/Prefabs/GroupTemplates/";

	public const string AudioSourceTemplateFolder = "Assets/DarkTonic/MasterAudio/Sources/Prefabs/AudioSourceTemplates/";
	...
	public static Transform AudioListenerTransform
	{
		get
		{
			...
			_audListenerTrans = audioListener.transform;
			return _audListenerTrans;
		}
	}
	...
	public static MasterAudio SafeInstance
	{
		get
		{
			return (MasterAudio)UnityEngine.Object.FindObjectOfType(typeof(MasterAudio));
		}
	}
	...
	private void Awake()
	{
		...
		for (int num = 0; num < musicPlaylists.Count; num++)
		{
			...
			for (int num2 = 1; num2 < playlist.MusicSettings.Count; num2++)
			{
				...
				{
					Debug.LogWarning("Playlist '" + playlist.playlistName + "' is marked as Synchronized but the clip lengths are not all the same within the Playlist. Clips may not synchronize as intended.");
					break;
				}
			}
		}
	...
	private void PerformBusFades()
	{
		for (int i = 0; i < BusFades.Count; i++)
		{
			...
			if (actingBus == null)
			{
				Debug.Log("Could not find bus named '" + busFadeInfo.NameOfBus + "' to fade it one step.");
	...
	private void PerformGroupFades()
	{
		for (int i = 0; i < GroupFades.Count; i++)
		{
			...
			if (actingGroup == null)
			{
				Debug.Log("Could not find Sound Group named '" + groupFadeInfo.NameOfGroup + "' to fade it one step.");
	...
	private static PlaySoundResult PlaySoundAtVolume(string sType, float volumePercentage, Vector3 sourcePosition, float? pitch = null, Transform sourceTrans = null, string variationName = null, bool attachToSource = false, float delaySoundTime = 0f, bool useVector3 = false, bool makePlaySoundResult = false, bool isChaining = false, bool isSingleSubscribedPlay = false, bool triggeredAsChildGroup = false)
	{
		...
		if (SoloedGroups.Count > 0 && !SoloedGroups.Contains(masterAudioGroup))
		{
			if (flag)
			{
		...
		if (masterAudioGroup.curVariationMode == MasterAudioGroup.VariationMode.Normal)
		{
			...
			case MasterAudioGroup.LimitMode.TimeBased:
				if (!(masterAudioGroup.minimumTimeBetween > 0f))
				{
					break;
				}
				if (Time.realtimeSinceStartup < audioGroupInfo.LastTimePlayed + masterAudioGroup.minimumTimeBetween)
				{
					if (flag)
					{
		...
		if (audioInfo == null)
		{
			...
			if (flag2)
			{
				...
				randomIndex = ((audioGroupInfo.Group.curVariationSequence != MasterAudioGroup.VariationSequence.Randomized) ? new int?(0) : new int?(UnityEngine.Random.Range(0, list.Count)));
		...
		else if (audioGroupInfo.Group.childGroupMode == MasterAudioGroup.ChildGroupMode.TriggerLinkedGroupsWhenPlayed && !triggeredAsChildGroup && !IsWarming)
		{
	...
	public static void FadeSoundGroupToVolume(string sType, float newVolume, float fadeTime, Action completionCallback = null)
	{
		...
		float volumeStep = (newVolume - masterAudioGroup.groupMasterVolume) / (fadeTime / 0.1f);
		...
		for (int i = 0; i < aGroup.groupVariations.Count; i++)
		{
			...
			for (int j = 0; j < dynamicGroupVariation.weight; j++)
			{
				...
				case AudioLocation.ResourceFile:
					...
					component.audLocation = AudioLocation.ResourceFile;
	...
	public static void MuteBus(string busName)
	{
		...
		while (enumerator.MoveNext())
		{
			...
			if (masterAudioGroup.busIndex == busIndex)
			{
				MuteGroup(masterAudioGroup.GameObjectName);
			}
		}
	}
	...
	public static void UnmuteBus(string busName)
	{
		...
		while (enumerator.MoveNext())
		{
			...
			if (masterAudioGroup.busIndex == busIndex)
			{
				UnmuteGroup(masterAudioGroup.GameObjectName);
			}
		}
	}
	...
	public static void SoloBus(string busName)
	{
		...
		while (enumerator.MoveNext())
		{
			...
			if (masterAudioGroup.busIndex == busIndex)
			{
				SoloGroup(masterAudioGroup.GameObjectName);
			}
		}
	}
	...
	public static void UnsoloBus(string busName)
	{
		...
		while (enumerator.MoveNext())
		{
			...
			if (masterAudioGroup.busIndex == busIndex)
			{
				UnsoloGroup(masterAudioGroup.GameObjectName);
			}
		}
	}
		...
		while (enumerator.MoveNext())
		{
			...
			if (masterAudioGroup.busIndex != -1)
			{
				if (masterAudioGroup.busIndex == busIndex)
				{
					...
					Instance.RouteGroupToUnityMixerGroup(masterAudioGroup.name, null);
				}
	...
	public static void FadeBusToVolume(string busName, float newVolume, float fadeTime, Action completionCallback = null)
	{
		...
		float volumeStep = (newVolume - groupBus.volume) / (fadeTime / 0.1f);
		...
		foreach (string key in AudioSourcesBySoundType.Keys)
		{
			...
			for (int i = 0; i < audioGroupInfo.Sources.Count; i++)
			{
				...
				if (source.isPlaying)
				{
					...
					float volume = audioInfo.OriginalVolume * audioInfo.LastPercentageVolume * num + audioInfo.LastRandomVolume;
					source.volume = volume;
	...
	public static void ChangePlaylistPitch(string playlistName, float pitch, string songName = null)
	{
		...
		for (int i = 0; i < playlist.MusicSettings.Count; i++)
		{
			...
			if (string.IsNullOrEmpty(songName) || !(musicSetting.songName != songName))
			{
	...
	public static void AddCustomEventReceiver(ICustomEventReceiver receiver, Transform receiverTrans)
	{
		...
		for (int i = 0; i < Instance.customEvents.Count; i++)
		{
			...
			if (!ReceiversByEventName.ContainsKey(customEvent.EventName))
			{
				ReceiversByEventName.Add(customEvent.EventName, new Dictionary<ICustomEventReceiver, Transform> { { receiver, receiverTrans } });
				continue;
			}
	...
	public static void FireCustomEvent(string customEventName, Vector3 originPoint)
	{
		...
		if (customEventByName.frameLastFired >= Time.frameCount)
		{
	...
	{
		UnityEngine.Object obj = Resources.Load("Assets/DarkTonic/MasterAudio/Prefabs/MasterAudio.prefab", typeof(GameObject));
		if (obj == null)
		{
	...
	{
		UnityEngine.Object obj = Resources.Load("Assets/DarkTonic/MasterAudio/Prefabs/PlaylistController.prefab", typeof(GameObject));
		if (obj == null)
		{
	...
	{
		UnityEngine.Object obj = Resources.Load("Assets/DarkTonic/MasterAudio/Prefabs/DynamicSoundGroupCreator.prefab", typeof(GameObject));
		if (obj == null)
		{
	...
	{
		UnityEngine.Object obj = Resources.Load("Assets/DarkTonic/MasterAudio/Prefabs/SoundGroupOrganizer.prefab", typeof(GameObject));
		if (obj == null)
		{
