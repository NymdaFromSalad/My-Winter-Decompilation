...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class AxisEvent : FsmStateAction
	{
		...
		{
			float num = ((!(horizontalAxis.Value != string.Empty)) ? 0f : cInput.GetAxis(horizontalAxis.Value));
			float num2 = ((!(verticalAxis.Value != string.Empty)) ? 0f : cInput.GetAxis(verticalAxis.Value));
