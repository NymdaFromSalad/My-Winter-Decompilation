...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class GUILayoutBeginAreaFollowObject : FsmStateAction
	{
		...
		public override void OnGUI()
		{
			...
			float left = vector.x + ((!normalized.Value) ? offsetLeft.Value : (offsetLeft.Value * (float)Screen.width));
			float top = vector.y + ((!normalized.Value) ? offsetTop.Value : (offsetTop.Value * (float)Screen.width));
			Rect screenRect = new Rect(left, top, width.Value, height.Value);
			if (normalized.Value)
			{
