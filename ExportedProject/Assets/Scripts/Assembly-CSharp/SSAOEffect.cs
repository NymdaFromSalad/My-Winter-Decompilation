...
public class SSAOEffect : MonoBehaviour
{
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		...
		if (flag)
		{
			...
			m_SSAOMaterial.SetVector("_TexelOffsetScale", (!m_IsOpenGL) ? new Vector4((float)m_Blur / (float)source.width, 0f, 0f, 0f) : new Vector4(m_Blur, 0f, 1f / (float)m_Downsampling, 0f));
			m_SSAOMaterial.SetTexture("_SSAO", renderTexture);
			Graphics.Blit(null, temporary, m_SSAOMaterial, 3);
			RenderTexture.ReleaseTemporary(renderTexture);
			RenderTexture temporary2 = RenderTexture.GetTemporary(source.width, source.height, 0);
			m_SSAOMaterial.SetVector("_TexelOffsetScale", (!m_IsOpenGL) ? new Vector4(0f, (float)m_Blur / (float)source.height, 0f, 0f) : new Vector4(0f, m_Blur, 1f, 0f));
